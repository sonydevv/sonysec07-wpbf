...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='wpbf will audit and bruteforce your WordPress installation to test password strength, server configuration, users and installed plugins. It currently supports threads and HTTP proxy and provides a very small default wordlist (a dynamic wordlist is generated by default from the blog\'s content) and basic username detection.')
    parser.add_argument('url', type=str, help='base URL where WordPress is installed')
    parser.add_argument('-w', '--wordlist', default=config.wordlist, help="wordlist file (default: "+config.wordlist+")")
    ...
    parser.add_argument('-eu', '--enumerateusers', action="store_true", help="only enumerate users (without bruteforcing)")
    ...
    if args.nopluginscan:
        with open(config.plugins_list, "r") as f:
            plugins_list = [plugin.strip() for plugin in f.readlines()]
        plugins_list.extend(plugin for plugin in wp.find_plugins() if plugin)
    ...
    if wp.check_loginlockdown():
        ...
        try:
            with open(config.wordlist, "r") as f:
                wordlist.extend(w.strip() for w in f.readlines())
        except IOError:
        ...
        if args.nokeywords:
            ...
            wordlist.extend(w.strip() for w in wp.find_keywords_in_url(config.min_keyword_len, config.min_frequency, config.ignore_with))
    ...
    while task_queue.qsize() > 0:
        try:
            ...
            try:
                ...
                wps = 0.6
            print(f"{current_queue} tasks left / {round(1 / wps, 2)} tasks per second / {round(wps * current_queue / 60, 2)} min left")
